                    ...
                    if (e) {
                        ...
                        if (e.name)
                            for (let [jid] of global.reportes_solicitudes.filter(([number]) => number)) {
                                let data = (await conn.onWhatsApp(jid))[0] || {}
                                if (data.exists)
                                    await m.reply(`*[ ⚠️ 𝐂𝐎𝐌𝐀𝐍𝐃𝐎 𝐂𝐎𝐍 𝐅𝐀𝐋𝐋𝐎𝐒!!! ⚠️ ]*\n\n*En el plugin:* ${m.plugin}\n*Usuario:* ${m.sender}\n*Comando:* ${usedPrefix}${command} ${args.join(' ')}\n\n\`\`\`${text}\`\`\`\n\n*REPORTADO A LOS COLABORADORES Y CREADOR DEL BOT!!!*`.trim(), data.jid)
                            }
                        await m.reply(text)
                    }
                } finally {
...
export async function callUpdate(callUpdate) {
    ...
    if (nk.isGroup == false) {
    if (nk.status == "offer") {
    let callmsg = await this.reply(nk.from, `𝐇𝐨𝐥𝐚 *@${nk.from.split('@')[0]}*, 𝐥𝐚𝐬 ${nk.isVideo ? '𝐯𝐢𝐝𝐞𝐨𝐥𝐥𝐚𝐦𝐚𝐝𝐚𝐬' : '𝐥𝐥𝐚𝐦𝐚𝐝𝐚𝐬'} 𝐧𝐨 𝐞𝐬𝐭á𝐧 𝐩𝐞𝐫𝐦𝐢𝐭𝐢𝐝𝐚𝐬, 𝐬𝐞𝐫á𝐬 𝐛𝐥𝐨𝐪𝐮𝐞𝐚𝐝𝐨.\n-\n𝐒𝐢 𝐚𝐜𝐜𝐢𝐝𝐞𝐧𝐭𝐚𝐥𝐦𝐞𝐧𝐭𝐞 𝐥𝐥𝐚𝐦𝐚𝐬𝐭𝐞 𝐩ó𝐧𝐠𝐚𝐬𝐞 𝐞𝐧 𝐜𝐨𝐧𝐭𝐚𝐜𝐭𝐨 𝐜𝐨𝐧 𝐦𝐢 𝐜𝐫𝐞𝐚𝐝𝐨𝐫 𝐩𝐚𝐫𝐚 𝐪𝐮𝐞 𝐭𝐞 𝐝𝐞𝐬𝐛𝐥𝐨𝐪𝐮𝐞𝐞!`, false, { mentions: [nk.from] })
    //let data = global.owner.filter(([id, isCreator]) => id && isCreator)
    //await this.sendContact(nk.from, data.map(([id, name]) => [id, name]), false, { quoted: callmsg })
    let vcard = `BEGIN:VCARD\nVERSION:3.0\nN:;🆄🅽🆂🅲 🅱🅾🆃;;;\nFN:🆄🅽🆂🅲 🅱🅾🆃\nORG:🆄🅽🆂🅲 🅱🅾🆃\nTITLE:\nitem1.TEL;waid=5219992095479:+521 999 209 5479\nitem1.X-ABLabel:🆄🅽🆂🅲 🅱🅾🆃\nX-WA-BIZ-DESCRIPTION:[❗] ᴄᴏɴᴛᴀᴄᴛᴀ ᴀ ᴇsᴛᴇ ɴᴜᴍ ᴘᴀʀᴀ ᴄᴏsᴀs ɪᴍᴘᴏʀᴛᴀɴᴛᴇs.\nX-WA-BIZ-NAME:🆄🅽🆂🅲 🅱🅾🆃\nEND:VCARD`
    await this.sendMessage(nk.from, { contacts: { displayName: '🆄🅽🆂🅲 🅱🅾🆃', contacts: [{ vcard }] }}, {quoted: callmsg})
...
global.dfail = (type, m, conn) => {
    let msg = {
        rowner: '*┊↳🎖️┊ 𝐄𝐬𝐭𝐞 𝐜𝐨𝐦𝐚𝐧𝐝𝐨 𝐬𝐨𝐥𝐨 𝐩𝐮𝐞𝐝𝐞 𝐬𝐞𝐫 𝐮𝐬𝐚𝐝𝐨 𝐩𝐨𝐫 𝐞𝐥 𝐏𝐫𝐨𝐩𝐢𝐞𝐭𝐚𝐫𝐢𝐨/𝐎𝐰𝐧𝐞𝐫 𝐝𝐞𝐥 𝐛𝐨𝐭 ♛*',
        owner: '*┊↳🎖️┊ 𝐄𝐬𝐭𝐞 𝐜𝐨𝐦𝐚𝐧𝐝𝐨 𝐬𝐨𝐥𝐨 𝐩𝐮𝐞𝐝𝐞 𝐬𝐞𝐫 𝐮𝐬𝐚𝐝𝐨 𝐩𝐨𝐫 𝐞𝐥 𝐏𝐫𝐨𝐩𝐢𝐞𝐭𝐚𝐫𝐢𝐨/𝐎𝐰𝐧𝐞𝐫 𝐝𝐞𝐥 𝐛𝐨𝐭♛*',
        mods: '*┊↳🎖️┊ 𝐄𝐬𝐭𝐞 𝐜𝐨𝐦𝐚𝐧𝐝𝐨 𝐬𝐨𝐥𝐨 𝐩𝐮𝐞𝐝𝐞 𝐬𝐞𝐫 𝐮𝐬𝐚𝐝𝐨 𝐩𝐨𝐫 𝐌𝐨𝐝𝐞𝐫𝐚𝐝𝐨𝐫𝐞𝐬 𝐲 𝐎𝐰𝐧𝐞𝐫 𝐝𝐞𝐥 𝐛𝐨𝐭 ♛*',
        premium: '*┊↳🎖️┊ 𝐄𝐬𝐭𝐞 𝐜𝐨𝐦𝐚𝐧𝐝𝐨 𝐬𝐨𝐥𝐨 𝐩𝐮𝐞𝐝𝐞 𝐬𝐞𝐫 𝐮𝐬𝐚𝐝𝐨 𝐩𝐨𝐫 𝐔𝐬𝐮𝐚𝐫𝐢𝐨𝐬 𝐏𝐫𝐞𝐦𝐢𝐮𝐦 𝐲 𝐞𝐥 𝐎𝐰𝐧𝐞𝐫 𝐝𝐞𝐥 𝐛𝐨𝐭 ♛*',
        group: '*┊↳🎖️┊ 𝐄𝐬𝐭𝐞 𝐜𝐨𝐦𝐚𝐧𝐝𝐨 𝐬𝐨𝐥𝐨 𝐩𝐮𝐞𝐝𝐞 𝐬𝐞𝐫 𝐮𝐭𝐢𝐥𝐢𝐳𝐚𝐝𝐨 𝐞𝐧 𝐠𝐫𝐮𝐩𝐨𝐬 ♛*',
        private: '*┊↳🎖️┊ 𝐄𝐬𝐭𝐞 𝐜𝐨𝐦𝐚𝐧𝐝𝐨 𝐬𝐨𝐥𝐨 𝐩𝐮𝐞𝐝𝐞 𝐬𝐞𝐫 𝐮𝐬𝐚𝐝𝐨 𝐞𝐧 𝐞𝐥 𝐩𝐫𝐢𝐯𝐚𝐝𝐨 𝐝𝐞𝐥 𝐛𝐨𝐭 ♛*',
        admin: '*┊↳🎖️┊ 𝐄𝐬𝐭𝐞 𝐜𝐨𝐦𝐚𝐧𝐝𝐨 𝐬𝐨𝐥𝐨 𝐩𝐮𝐞𝐝𝐞 𝐬𝐞𝐫 𝐮𝐢𝐥𝐢𝐳𝐚𝐝𝐨 𝐩𝐨𝐫 𝐀𝐝𝐦𝐢𝐧𝐬 𝐝𝐞𝐥 𝐠𝐫𝐮𝐩𝐨 ♛*',
        botAdmin: '*┊↳🎖️┊ 𝐇𝐄𝐘!! 𝐍𝐞𝐜𝐞𝐬𝐢𝐭𝐨 𝐭𝐞𝐧𝐞𝐫 𝐀𝐝𝐦𝐢𝐧 𝐩𝐚𝐫𝐚 𝐩𝐨𝐝𝐞𝐫 𝐮𝐭𝐢𝐥𝐢𝐳𝐚𝐫 𝐞𝐬𝐭𝐞 𝐜𝐨𝐦𝐚𝐧𝐝𝐨!! ♛*',
    ...
    let prep = generateWAMessageFromContent(m.chat, { extendedTextMessage: { text: msg, contextInfo: { externalAdReply: { title: '[ ⚠ ] 𝐀𝐕𝐈𝐒𝐎 - 𝐀𝐋𝐄𝐑𝐓𝐀', body: 'ᴛʜᴇ ᴍʏsᴛɪᴄ - ʙᴏᴛ', thumbnail: imagen1, sourceUrl: 'https://github.com/ToxiPain/UNSC-MD.git' }}}}, aa)
